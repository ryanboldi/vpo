...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # MaxRL (Tajwar & Zeng et al., arXiv:2602.02710): GRPO with std → mean
        # in the normalizer. Single-solution scalar baseline; data/reward path
        # is identical to GRPO.
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.MULTI_RLVR:
        # Multi-solution scalar-reward baseline: the reward function aggregates
        # the (m', k) score matrix into a scalar (sum of every entry), then the
        # advantage path is the same as standard GRPO on that scalar.
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
        ...
        # VPO: per-rollout E_w[max_s w·r_s] over its own m-pool. No peer
        # coupling. Directly optimizes the test-time E_w[best-of-m] metric.
        ...
        # meta_info, NOT non_tensor_batch: a plain dict of batch-level scalars
        # has no leading batch dim, and DataProto.chunk asserts every
        # non_tensor value is an np.ndarray (legacy worker dispatch crashes).
        data.meta_info["_vpo_diag"] = vpo_diag
    elif adv_estimator == AdvantageEstimator.VPO_SINGLE:
        # VPO, single-solution: each rollout has m=1 so own-pool max
        # degenerates to w·r itself; advantage is the GRPO-style z-norm of
        # E_w[w·r] across the n rollouts in the group.
        from vpo.utils.vpo import vpo_advantage

        raw = data.non_tensor_batch["sub_scores"]
        sub_scores = [[list(r)] for r in raw]
        index = data.non_tensor_batch["uid"]
        advantages, vpo_diag = vpo_advantage(sub_scores, index)
        advantages = advantages.unsqueeze(-1).to(data.batch["response_mask"].device)
        advantages = advantages * data.batch["response_mask"]
        data.batch["advantages"] = advantages
        data.batch["returns"] = advantages
        data.meta_info["_vpo_diag"] = vpo_diag  # see VPO branch above
    else:
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path):
        ...
        # JSON encoder that handles numpy scalars/arrays. The training-time
        # dump path includes batch.non_tensor_batch fields (e.g. request_id)
        # which are numpy.int64 by default and crash json.dumps.
        def _default(o):
    ...
    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns):
        # Pool metrics from sub_scores (works for all methods)
        pool_metric_dict = {}
        if "sub_scores" in reward_extra_infos_dict and len(reward_extra_infos_dict["sub_scores"]) > 0:
            from vpo.utils.pool_metrics import build_pool_and_compute

            pool_metric_dict = build_pool_and_compute(
                reward_extra_infos_dict["sub_scores"],
                sample_uids,
            )

        # Filter non-scalar keys before passing to process_validation_metrics
        # (sub_scores is list-of-lists and would crash np.mean)
        _non_scalar_keys = {"sub_scores"}
        filtered_infos = {
            k: v for k, v in reward_extra_infos_dict.items()
            if k not in _non_scalar_keys
        }

        data_src2var2metric2val = process_validation_metrics(data_sources, sample_uids, filtered_infos)
        metric_dict = {}
        ...
        # Add pool metrics under val-aux prefix
        for k, v in pool_metric_dict.items():
            metric_dict[f"val-aux/{k}"] = v
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                        ...
                        if reward_extra_infos_dict:
                            for k, v in reward_extra_infos_dict.items():
                                try:
                                    ...
                                    # Ragged arrays (e.g. sub_scores with variable m per rollout)
                ...
                # Multi-solution diversity metrics
                _multi_sol_estimators = (
                    AdvantageEstimator.MULTI_RLVR,
                    AdvantageEstimator.VPO,
                )
                if self.config.algorithm.adv_estimator in _multi_sol_estimators:
                    for key in ("num_unique", "num_parsed"):
                        if key in batch.non_tensor_batch:
                            vals = np.asarray(batch.non_tensor_batch[key], dtype=np.float32)
                            metrics[f"diversity/{key}/mean"] = float(np.mean(vals))
                            metrics[f"diversity/{key}/max"] = float(np.max(vals))
                            metrics[f"diversity/{key}/min"] = float(np.min(vals))
                    for key in ("any_solution_solves", "first_solution_solves"):
                        if key in batch.non_tensor_batch:
                            vals = np.asarray(batch.non_tensor_batch[key], dtype=np.float32)
                            metrics[f"diversity/{key}/mean"] = float(np.mean(vals))
                    for key in ("score_sum_all", "score_max_sum_naive", "score_set_coverage"):
                        if key in batch.non_tensor_batch:
                            vals = np.asarray(batch.non_tensor_batch[key], dtype=np.float32)
                            metrics[f"diversity/{key}/mean"] = float(np.mean(vals))
                # VPO (own-pool E_w[max_s w·r_s] per rollout) diagnostics
                if self.config.algorithm.adv_estimator in (
                    AdvantageEstimator.VPO,
                    AdvantageEstimator.VPO_SINGLE,
                ):
                    vpo_diag = batch.meta_info.get("_vpo_diag", None)
                    if isinstance(vpo_diag, dict):
                        metrics.update(vpo_diag)
                # Unified pool metrics (all methods with sub_scores)
                if "sub_scores" in batch.non_tensor_batch and "uid" in batch.non_tensor_batch:
                    from vpo.utils.pool_metrics import build_pool_and_compute

                    pool_metrics = build_pool_and_compute(
                        batch.non_tensor_batch["sub_scores"],
