...
class AdvantageEstimator(str, Enum):
    ...
    GDPO = "gdpo"
    MULTI_RLVR = "multi_rlvr"
    VPO = "vpo"
    VPO_SINGLE = "vpo_single"
    MAX_AT_K = "max_at_k"
    MAXRL = "maxrl"
...
def compute_maxrl_outcome_advantage(
    ...
    # Per-prompt advantage = (r - mean) / (mean + eps), i.e. mean-subtract +
    # scale by 1/p_hat. For binary reward this matches the (1/K) * sum_i r_i
    # * S_i gradient up to a constant baseline; works for continuous reward
    # too. When all rollouts fail (mean = 0) the advantage is 0 → no update.
    scores = token_level_rewards.sum(dim=-1)
    id2score = defaultdict(list)
    id2mean = {}
...
@register_adv_est(AdvantageEstimator.MAX_AT_K)  # or simply: @register_adv_est("max_at_k")
def compute_max_at_k_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    **kwargs,
    ...
    """Unbiased gradient estimator for E[max reward in size-k subset of n rollouts].

    Implements the on-policy max@k estimator from
    https://arxiv.org/abs/2510.23393 ("The Best of N Worlds").

    For each prompt's group of n rollouts with scalar rewards r_1..r_n,
    sort ascending so rank-i (1-indexed) has the i-th smallest reward. Then:

        w_ii   = C(rank_i - 1, k - 1)        # rank_i is the max of its size-k subset
        w_ij   = C(rank_j - 2, k - 2) for j>i in rank order
        A_i    = (sum_j w_ij * r_j) / C(n, k)

    GRPO-style per-group mean (and optional std) normalization is then applied
    so the estimator is directly comparable to GRPO. k is read from
    `config.max_at_k_k`; defaults to max(2, n // 2). k = n is degenerate after
    mean subtraction (every sample's advantage equals the group max), so we
    clamp to k <= n - 1.
    """
    assert config is not None
    norm_adv_by_std_in_grpo = config.get("norm_adv_by_std_in_grpo", True)
    k_cfg = config.get("max_at_k_k", None)
    ...
    with torch.no_grad():
        ...
        for idx, idxs in id2indices.items():
            n = len(idxs)
            if n < 2:
                continue
            k = k_cfg if k_cfg is not None else max(2, n // 2)
            if k < 1 or k >= n:
                raise ValueError(
                    f"max@k requires 1 <= k <= n - 1; got k={k}, n={n} for group {idx}. "
                    f"k=n is degenerate under group mean subtraction (advantage collapses to 0)."
                )
            r = torch.stack([scores[j] for j in idxs])
            sorted_r, sort_idx = torch.sort(r, dim=0)
            denom = float(comb(n, k))

            rank_adv = torch.zeros(n, dtype=scores.dtype, device=scores.device)
            for ri in range(n):
                rank1 = ri + 1
                acc = float(comb(rank1 - 1, k - 1)) * float(sorted_r[ri])
                if k >= 2:
                    for rj in range(ri + 1, n):
                        w = comb(rj - 1, k - 2)  # rj is 0-indexed → rank j-1, so j-2 = rj-1
                        if w:
                            acc += float(w) * float(sorted_r[rj])
                rank_adv[ri] = acc / denom

            group_adv = torch.empty(n, dtype=scores.dtype, device=scores.device)
            group_adv[sort_idx] = rank_adv

            mean = group_adv.mean()
            if norm_adv_by_std_in_grpo:
                std = group_adv.std()
                group_adv = (group_adv - mean) / (std + epsilon)
            else:
                group_adv = group_adv - mean

            for j_local, orig_pos in enumerate(idxs):
                advantages[orig_pos] = group_adv[j_local]
