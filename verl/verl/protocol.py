    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        for key, val in non_tensor_batch.items():
            try:
                ...
                # Handle mixed dimensions (e.g. object arrays with ragged sub_scores)
                flat = []
                for v in val:
                    if v.ndim == 0:
                        flat.append(v.item())
                    else:
                        flat.extend(v.tolist() if hasattr(v, 'tolist') else list(v))
        ...
        for key, val in non_tensor_batch.items():
            if not isinstance(val, np.ndarray):
                try:
                    ...
                    arr[:] = val
                    val = arr
            # Skip 0-d arrays (scalars) — they can't be iterated
            if val.ndim == 0:
                continue
